    ...
    <div className="min-h-[calc(100vh-56px)] flex">
      ...
      >
        <div className="text-center max-w-md px-8">
          ...
          <h2 className="text-3xl xl:text-4xl font-bold text-[#1C1917] mb-3">Duygularınla çiz</h2>
          ...
          <div className="flex justify-center gap-3 mt-8">
            {["🌊","⚡","🌸","🔥","🌙"].map(e => (
              <span key={e} className="w-10 h-10 rounded-full bg-white/60 flex items-center justify-center text-xl shadow-sm">{e}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-8 py-10">
        <div className="w-full max-w-sm">
          ...
          <div className="flex flex-col items-center mb-8 lg:hidden">
            <div className="w-16 h-16 rounded-2xl bg-[#D9723F] flex items-center justify-center mb-4 shadow-md">
              <Image src="/logo.png" alt="Vibe" width={40} height={40} className="rounded-xl" />
            </div>
          </div>

          <h1 className="text-2xl font-bold text-[#1C1917] text-center lg:text-left">
            {mode === "login" ? "Tekrar hoş geldin" : "Vibe&apos;a katıl"}
          </h1>
          <p className="text-sm text-[#78716C] mt-1.5 text-center lg:text-left mb-6">
            {mode === "login" ? "Hesabına giriş yap" : "Ücretsiz hesap oluştur"}
          </p>

          {mode === "signup" && (
            <div className="flex items-center gap-1.5 mb-6">
              <div className={cn("flex-1 h-1.5 rounded-full transition-colors", step >= 1 ? "bg-[#D9723F]" : "bg-[#E8E4DC]")} />
              <div className={cn("flex-1 h-1.5 rounded-full transition-colors", step === 2 ? "bg-[#D9723F]" : "bg-[#E8E4DC]")} />
            </div>
          )}

          <div className="bg-white border border-[#E8E4DC] rounded-[22px] p-6 shadow-sm">
            {mode === "login" && (
              <div className="flex flex-col gap-4">
                ...
                <button onClick={handleLogin} disabled={loading}
                  className="w-full py-3 bg-[#D9723F] text-white rounded-[14px] font-semibold text-sm hover:bg-[#C4622F] disabled:opacity-50 transition-all active:scale-[0.98]">
            ...
            {mode === "signup" && step === 1 && (
              <div className="flex flex-col gap-4">
                <p className="text-xs font-semibold text-[#A8A29E] uppercase tracking-widest">Adım 1 / 2</p>
                ...
                <button onClick={() => { if (!displayName || !email || password.length < 6) { setError("Tüm alanları doldur."); return }; setError(""); setStep(2) }}
                  className="w-full py-3 bg-[#D9723F] text-white rounded-[14px] font-semibold text-sm hover:bg-[#C4622F] transition-all active:scale-[0.98]">
                  Devam
                </button>
              </div>
            )}
            ...
            {mode === "signup" && step === 2 && (
              <div className="flex flex-col gap-5">
                <p className="text-xs font-semibold text-[#A8A29E] uppercase tracking-widest">Adım 2 / 2</p>
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-xs text-[#A8A29E]">Avatarın</p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-[#78716C] mb-2.5">Emoji seç</p>
                  <div className="grid grid-cols-6 gap-2">
                    {EMOJIS.map(e => (
                      <button key={e} onClick={() => setEmoji(e)} className={cn(
                        "h-10 rounded-[10px] text-xl transition-all active:scale-90",
                        emoji === e ? "bg-[#D9723F]/15 ring-2 ring-[#D9723F]/50 scale-110" : "bg-[#F5F3EF] hover:bg-[#EDE9E3]"
                      )}>{e}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <p className="text-xs font-semibold text-[#78716C] mb-2.5">Renk seç</p>
                  <div className="flex flex-wrap gap-2.5">
                    {COLOR_KEYS.map(k => (
                      <button key={k} onClick={() => setColor(k)} className={cn(
                        "w-7 h-7 rounded-full transition-all",
                        color === k ? "scale-125 ring-2 ring-offset-2 ring-[#D9723F]" : "hover:scale-110"
                ...
                <div className="flex gap-2">
                  <button onClick={() => setStep(1)}
                    className="flex-1 py-3 bg-white border border-[#E8E4DC] text-[#1C1917] rounded-[14px] font-semibold text-sm hover:bg-[#F5F3EF] transition-colors">
                    Geri
                  </button>
                  <button onClick={handleSignup} disabled={loading}
                    className="flex-1 py-3 bg-[#D9723F] text-white rounded-[14px] font-semibold text-sm hover:bg-[#C4622F] disabled:opacity-50 transition-all">
          ...
          <p className="text-center text-sm text-[#78716C] mt-5">
            ...
            <button onClick={() => { setMode(mode === "login" ? "signup" : "login"); setStep(1); setError("") }}
              className="text-[#D9723F] font-medium hover:underline">
  ...
  return (
    <div>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className="w-full px-4 py-3 rounded-[14px] bg-[#FAF8F4] border border-[#E8E4DC] text-[#1C1917] text-sm placeholder:text-[#A8A29E] focus:outline-none focus:ring-2 focus:ring-[#D9723F]/20 focus:border-[#D9723F] transition" />
    </div>
  )
}
